...
module.exports.local_login_post = async (req, res) => {
  try {
    const { identifier, password } =
      await joiSchemas.local_login_post.validateAsync(req.body);
...
module.exports.change_password_post = async (req, res) => {
  try {
    const results = await joiSchemas.change_password_post.validateAsync(
      req.body
    );
    isPasswordValid = await UserService.validatePassword(
      req.user._id,
...
module.exports.forgot_password_post = async (req, res) => {
  try {
    const results = await joiSchemas.forgot_password_post.validateAsync(
      req.body
    );
...
module.exports.reset_password_post = async (req, res) => {
  try {
    const results = await joiSchemas.reset_password_post.validateAsync(
      req.body
    );
    let user = await UserService.readByEmail(results.email);
    if (!user) return res.fail("User with this email is not Signup yet");
    if (
      user.forgotPasswordCode !== Number(results.forgotPasswordCode) ||
...
module.exports.getAllUsers = async (req, res) => {
  try {
    ...
    console.error("AuthController [getAllUsres] Error", error);
...
module.exports.getSingleUser = async (req, res) => {
  try {
    ...
    console.error("AuthController [getSingleUser] Error", error);
    return res.status(500).json({
      message: "Internal Srever Error",
    });
  }
